...
static [PsCustomObject] FindModules ()
{
    try 
    {
        ...
        $searchScope = Compare-Object $requiredModules $currentModules -Property ModuleName,ModuleVersion |  
        Where-Object {$Psitem.Sideindicator -eq '<=' -and $PsItem.ModuleName -ne 'PsDesiredStateConfiguration'}
...
{
    foreach ($node in $nodes.Where{$Psitem -ne $env:COMPUTERNAME})
    {
        try 
        {
            ...
            { 
            Copy-Item -Path "C:\Program Files\WindowsPowerShell\Modules\$Module" -ToSession $CurrentSession -Destination "C:\Program Files\WindowsPowerShell\$Module" -Force -Recurse
            }
        }
        catch
        {
...
if ($prep -eq $true){
    $Nodes = [Vamp]::InitalizeNodes()
    [Vamp]::BootstrapNuget()
    $ToDownload = [Vamp]::FindModules()
    [Vamp]::DownloadModules($ToDownload)
    [Vamp]::CopyModules($Nodes, $ToDownload.Name)
}
if ($apply -eq $true) 
{
    [Vamp]::Main()
